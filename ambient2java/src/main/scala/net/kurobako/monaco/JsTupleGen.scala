      ...
      case ts @ (_ :: _) =>
        ...
        (s"${pkg.replace('.', '/')}/$clsName$n.java" ->
          s"""package $pkg;
             ...
             |$getters
             |
             |	@Override public List<Object> elements() { return Arrays.asList(${varNames.mkString(", ")}); }
             |
             ...
             |	}
             |	@Override public int hashCode() { return Objects.hash(${varNames.mkString(", ")}); }
             |	@Override public String toString() { return "$clsName$n(" + ${varNames.mkString("""+ ", " +""")} + ')'; }
             |}
             |
             |""".stripMargin).pureF
    }
