...
object TypeDocParser {
  ...
  implicit class RichValue(private val v: Value) extends AnyVal {
    ...
    def !![A](key: String, f: Value => Option[A]): ParseM[A] =
      (v !! key) >>= (f(_).otherwise(s"Missing `$key` for dictionary $v"))
  ...
  def parseTopLevel(value: Value, kind: ReflectionKind): ParseM[TopLevel] = detailed(value, "TopLevel")(for {
    id       <- value.!![Int]("id", _.intOpt)
    rawName  <- value.!![String]("name", _.strOpt)
    name     <- Sym(rawName)
    ...
    }.map(_.collect {case (x, true) => x }.toSet)
  ...
  def parseType(tpeDictName: Option[String], value: Value, ctx: Option[Meta]): ParseM[TsType[TsName]] =
    detailed(value, "Type")(for {
      ...
      tpe     <- tpeKind match {
        ...
        case TypeKind.Union     => for {
            components <- obj.!!("types", _.listOpt)
            xs         <- components.traverse(parseType(none, _, ctx))
          } yield TsUnion(xs)
        case TypeKind.Tuple => for {
            components <- obj.!!("elements", _.listOpt)
            xs         <- components.traverse(parseType(none, _, ctx))
          } yield TsTuple(xs)
        case TypeKind.Intersection => for {
            types <- obj.!!("types", _.listOpt)
            xs    <- types.traverse(parseType(none, _, ctx))
          } yield TsIntersection(xs)
        case TypeKind.StringLiteral => obj.!!("value", _.strOpt).map(TsStringLiteral(_)).map(TsTerminal(_))
        case TypeKind.TypeParameter => for {
            name <- obj.!!("name", _.strOpt)
            raw  <- Sym(name)
          } yield TsTerminal(TsTypeVar(raw))
  ...
  def parseKind(value: Value): ParseM[ReflectionKind] =
    value.!!("kind", _.intOpt) >>= (rawKind =>
      ReflectionKind(rawKind).otherwise(s"Unknown kind $rawKind for $value")
    )
  ...
  def parseVariable(
      enclosing: Name,
      tl: TopLevel,
      srcLns: ArraySeq[String]
  ): ParseM[TsVariable[TsType[TsName]]] = detailed(tl.value, "Variable") {
    parseType("type".some, tl.value, none).map { tpe =>
      TsVariable(
        enclosing :+ tl.meta.name,
        tl.meta.id,
        tpe,
        tl.meta.enabled(ReflectionFlag.Const),
        tl.meta.docLns(srcLns)
      )
    }
  }

  def parseFunctionLike(
      enclosing: Name,
      tl: TopLevel,
      name: String,
      ...
      ctx: Option[Meta],
      srcLns: ArraySeq[String]
  ): ParseM[(Name, Meta, List[TsSignature[TsType[TsName]]], Boolean)] = detailed(tl.value, name) {
      ...
      for {
        ...
        params <- parseKinds("parameters", ReflectionKind.Parameter)(sig.value) { param =>
          parseType("type".some, param.value, ctx).map(TsParameter(
            ...
            param.meta.enabled(ReflectionFlag.Optional),
            parseComment(param.value)
          ))
        }
  ...
  def parseTpeAlias(enclosing: Name, tl: TopLevel, srcLns: ArraySeq[String]): ParseM[TsTypeAlias[TsType[TsName]]] =
    detailed(tl.value, "TypeAlias") {
      parseType("type".some, tl.value, none).map { tpe =>
  ...
  def parseEnum(enclosing: Name, tl: TopLevel, srcLns: ArraySeq[String]): ParseM[TsEnum[TsType[TsName]]] =
    detailed(tl.value, "Enum") {
      for {
        partial <- parseKinds("children", ReflectionKind.EnumMember)(tl.value) { member =>
          member.value.!!("defaultValue", _.strOpt).map(v =>
            (v, TsEnumEntry(member.meta.name, _, member.meta.docLns(srcLns)))
          )
        }
  ...
  def parseClassLike(enclosing: Name, tl: TopLevel, srcLns: ArraySeq[String]): ParseM[
    ...
    methods <- parseKinds("children", ReflectionKind.Method)(tl.value) { method =>
      parseFunctionLike(enclosing, method, "Method", _ == method.meta.name, tl.meta.some, srcLns).map {
        case (_, meta, sigs, inherited) =>
          TsMethod(meta.name, sigs, meta.enabled(ReflectionFlag.Static), meta.docLns(srcLns), inherited)
      }
    }
  ...
  def parseClass(enclosing: Name, tl: TopLevel, srcLns: ArraySeq[String]): ParseM[TsClass[TsType[TsName]]] =
    detailed(tl.value, "Class") {
      for {
        ctors <- parseKinds("children", ReflectionKind.Constructor)(tl.value) { method =>
          parseFunctionLike(
            ...
            method,
            "Constructor",
            _ == Sym.unsafe(s"new ${tl.meta.name}"),
            tl.meta.some,
            srcLns
          ).map {
  ...
  def parseInterface(enclosing: Name, tl: TopLevel, srcLns: ArraySeq[String]): ParseM[TsInterface[TsType[TsName]]] =
    detailed(tl.value, "Interface") {
      for {
        (name, props, methods, parents, tpeParams) <- parseClassLike(enclosing, tl, srcLns)
      } yield TsInterface[TsType[TsName]](name, tl.meta.id, parents, tpeParams, methods, props, tl.meta.docLns(srcLns))
    }

  def parseModule(enclosing: Name, tl: TopLevel, srcLns: ArraySeq[String]): ParseM[List[TsModule[TsType[TsName]]]] =
    detailed(tl.value, "Module") {
  ...
  def parseGlobal(v: Value, srcLns: ArraySeq[String]): ParseM[TsGlobal[TsType[TsName]]] = {
    def requireKind(m: Meta)(is: ReflectionKind) =
      if (m.kind == is) ().pureF else failF(s"Expected $is but got ${m.kind}")
    for {
      kind                  <- parseKind(v)
  ...
  def apply(typeDocAstPath: File, tsSourcePath: File): ParseM[TsGlobal[TsType[TsName]]] = for {
    ...
    root2 = upickle.default.read[TypeDocParser2.TSReflection](tsDocAstStr, trace = true)
    _ <- TypeDocParser2.liftGlobal(root2, tsSource)
