...
object Ast {
  ...
  case class Sym(value: String) extends AnyVal {
    inline def ++(inline that: String): Sym = new Sym(value ++ that)
    override def toString: String           = s"'$value"
  }
  object Sym {
    given Show[Sym]                   = (t: Sym) => t.value
    given Order[Sym]                  = Order.by(_.value)
    def unsafe(s: String): Sym        = new Sym(s.replace("-", "_").ensuring(_.nonEmpty))
  ...
  object Name {
    given Order[Name]                           = Order.by(_.fragments.toVector)
