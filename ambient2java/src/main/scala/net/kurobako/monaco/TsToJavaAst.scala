...
import net.kurobako.monaco.JavaAst.JType._
import net.kurobako.monaco.JavaAst.JTypeTree.Foreign.Lit
import net.kurobako.monaco.JavaAst.JTypeTree._
import net.kurobako.monaco.JavaAst.{DelegationBody, _}
...
object TsToJavaAst {
	...
	val JOptional: Name = Name.cls[Optional[_]]
	...
	val JsContextCtors = List(
		JCtor((Sym.unsafe("context") -> JTypeContext(JavaRef(JsContext))) ::
			  (Sym.unsafe("receiver") -> JTypeContext(JavaRef(Name.cls[Object]))) :: Nil,
			DelegationBody.Raw("super(context, receiver);" :: Nil)),
		JCtor((Sym.unsafe("context") -> JTypeContext(JavaRef(JsContext))) :: Nil,
	...
	def flattenChoices[A](xs: List[JTypeTree[A]]): ParseM[Alt[A]] = alternatives(xs).map(x => Alt(x.flatten.distinctUniversal))

	def tsUnionOrIntersectionToJava(tpes: List[TsType[TsName]], lut: TpeLookup): ParseM[Partial] = {
		// crush (null | undefined) here

		def matchLits[A](xs: List[Partial])(f: PartialFunction[Foreign, Lit[A]]): Option[Nel[A]] = xs.foldLeftM(Vector[A]()) {
		...
		def liftInner(xs: List[TsType[TsName]], nullable: Boolean): ParseM[Partial] = for {
			...
			ys <- potentialEnums.cata(_.pureF, flattenChoices(xs).map(_.modify(_.copy(nullable = nullable))).elaborate(s"Union $tpes"))
		} yield ys
		...
		val params = paramDocs match {
			...
			case xs  => " * " :: xs.map { case (name, comment) =>
				...
				val reformatted = if (
					...
					text.startsWith("<p>") &&
					text.endsWith("</p>")) text.stripPrefix("<p>").stripSuffix("</p>")
				else text
	...
	def emitTpeBpunds(tpeParams: List[(Sym, Option[Nel[JTypeContext[Name]]])]): ParseM[List[(Sym, Option[JTypeContext[Name]])]] = {
		tpeParams.traverse { case (s, b) =>
			b.traverse {
				case x Nel Nil => x.pureF
				case xs        => failF(s"Bounds with alternatives: $xs")
			}.map(s -> _)
		}
	}

	def emitCallable(sigs: List[TsSignature[Partial]], name: Sym, parentDoc: Doc, static: Boolean, reservedNames: Set[Sym]): ParseM[List[JMethod[Name]]] =
		sigs.distinct.flatTraverse[ParseM, (List[JType[Name]], JMethod[Name])] {
			...
			methods.groupMap(_._1)(_._2).toList.flatMap { case (_, ms) =>
				val (optional, required) = ms.partitionMap(m => m.rtn match {
	...
	def emitFunction(fn: TsFunction[Partial]): CheckM[List[JMethod[Name]]] =
		emitCallable(fn.signatures, fn.name.sym, fn.doc, static = true, Set()).elaborate(emitError("function", fn)).toValidatedNel
	...
	def emitPropertyLike(name: Sym, tpe: Partial,
						 optional: Boolean, get: Boolean, set: Boolean,
						 doc: Doc, enclosing: JType[Name]): CheckM[List[JMethod[Name]]] = {
		...
		tpe match {
			case Alt(JTypeContext(JavaVoid, _) Nel Nil) => Nil.pureF
			case Alt(xs)                                =>
	...
	} yield (clsTpe, inheritedNames, (moreCtors: List[JCtor[Name]], moreMethods: List[JMethod[Name]]) => {
	...
	def emitClass(cls: TsClass[Partial], tpeLut: TpeLookup, nameLut: NameLookup): CheckM[JClass[Name]] = emitClassLike(cls, tpeLut, nameLut).andThen { case (clsTpe, inheritedNames, mkCls) =>
		...
		val accessors  = cls.accessors.flatTraverse { x =>
			val (get, set) = x.kind match {
				case TsAccessor.TsGetter          => (true, false)
				case TsAccessor.TsSetter          => (false, true)
				case TsAccessor.TsGetterAndSetter => (true, true)
			}
	...
	def emitInterface(iface: TsInterface[Partial], tpeLut: TpeLookup, nameLut: NameLookup): CheckM[JClass[Name]] = emitClassLike(iface, tpeLut, nameLut).andThen { case (clsTpe, inheritedNames, mkCls) =>
	...
	def emitModule(m: TsModule[Partial]): CheckM[JClass[Name]] = {
		...
		(
			m.modules.flatTraverse(m => emitPropertyLike(m.name.sym, Alt( JavaRef( moduleName(m.name)  )), optional = false, get = true, set = false, doc = m.doc,  JavaRef(name))) |+|
			m.variables.flatTraverse(v => emitPropertyLike(v.name.sym, v.tpe, optional = false, get = true, set = !v.const, v.doc, JavaRef(name))) |+|
	...
	def main(args: Array[String]): Unit = {
		...
		val a = for {
			...
			aliases = time("Isolate alias") {
				tsTypedGlobal.modules.flatMap(_.aliases).map(a => a.id -> (a.tpe match {
			...
			nameLut <- time("Create Name LUT") {
				safeGroup(tsAliasResolvedGlobal.modules.flatMap { m =>
					m.classes.map(c => c.name -> (c.methods.map(_.name) ++
												  c.properties.map(_.name) ++
												  c.accessors.map(_.name)).toSet) ++
					m.interfaces.map(i => i.name -> (i.methods.map(_.name) ++
